# =============================================================================
# Hybrid FDD rare-class fine-tune from FDD-protocol best.pt (~ep 235)
# Weights: Cos 1.5 | Lighter 3.0 | others 1.0
# Oversample: Cos×2 | Lighter×5
# imgsz=768, 40 epochs, AdamW lr0=2e-4
#
# Kaggle Inputs: 1) HiXray_YOLO2  2) best.pt (from results166)
# Accelerator: GPU T4 x2
# Run cells 0 → 6 in order
# =============================================================================
...
CELL_1 = r'''
# Cell 1 — paths: HiXray + FDD best.pt
import os
from pathlib import Path

WORK_DIR = '/kaggle/working/hybrid_fdd_rare_ft'
...
# Prefer best.pt (FDD results166). Avoid tiny/wrong files.
pts = [p for p in INPUT_ROOT.rglob('*.pt') if p.stat().st_size > 8_000_000]
pts.sort(key=lambda p: (
    0 if p.name == 'best.pt' else 1,
    0 if 'best' in str(p).lower() else 1,
    -p.stat().st_size,
))
assert pts, 'Add FDD best.pt (from results166) as Kaggle Input'
BEST_PT = str(pts[0])

print('WORK_DIR', WORK_DIR)
print('DATASET_PATH', DATASET_PATH)
print('START_CKPT', BEST_PT, f'({Path(BEST_PT).stat().st_size/1e6:.1f} MB)')
print('train OK', (Path(DATASET_PATH) / 'images' / 'train').is_dir())
print('val OK', (Path(DATASET_PATH) / 'images' / 'val').is_dir())
'''

CELL_2 = r'''
# Cell 2 — oversample Cosmetic×2, Lighter×5
from pathlib import Path
...
OVERSAMPLE = {0: 2, 3: 5}  # Cos×2, Lighter×5
...
TRAIN_TXT = Path(WORK_DIR) / 'train_rare_fdd.txt'
TRAIN_TXT.write_text('\n'.join(train_entries) + '\n', encoding='utf-8')

DATA_YAML = str(Path(WORK_DIR) / 'dataset.yaml')
names_block = '\n'.join(f'  {i}: {n}' for i, n in enumerate(CLASSES))
Path(DATA_YAML).write_text(
    f"# FDD rare fine-tune (Cosmetic×2, Lighter×5)\n"
...
CELL_5 = r'''
# Cell 5 — train: class weights Cos=1.5 Lighter=3.0, imgsz=768
...
RUN_NAME = 'hybrid_fdd_rare_ft'

INPUT_ROOT = Path('/kaggle/input')
pts = [p for p in INPUT_ROOT.rglob('*.pt') if p.stat().st_size > 8_000_000]
pts.sort(key=lambda p: (0 if p.name == 'best.pt' else 1, -p.stat().st_size))
START_CKPT = str(pts[0])
assert Path(START_CKPT).exists(), 'best.pt not found'
print('Finetune from FDD:', START_CKPT)
...
# Cos=1.5, Lighter=3.0, others=1.0
...
print('Done FDD rare fine-tune. EPOCHS=', EPOCHS, 'imgsz=', IMGSZ)
print('cls weights: Cos=1.5 Lighter=3.0 others=1.0')
print('best.pt criterion: (Cosmetic + Lighter) mAP50 / 2')

WEIGHTS_DIR = Path(PROJECT_DIR) / RUN_NAME / 'weights'
print('Weights dir:', WEIGHTS_DIR)
if WEIGHTS_DIR.exists():
    print('Saved files:', sorted(p.name for p in WEIGHTS_DIR.glob('*.pt')))
'''

CELL_6 = r'''
# Cell 6 — per-class eval after training
import sys
from pathlib import Path
...
model = YOLO(str(ckpt))
metrics = model.val(data=DATA_YAML, imgsz=IMGSZ, batch=16, conf=0.001, iou=0.5, device=0)
...
print('Compare to FDD best before FT: ALL~0.800 Cos~0.654 Lighter~0.017')
'''

if __name__ == '__main__':
    from pathlib import Path
    out = Path(__file__).with_name('fdd_rare_finetune_CELLS.txt')
    parts = []

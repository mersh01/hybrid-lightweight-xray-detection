# =============================================================================
# Kaggle — COPY THIS ENTIRE CELL
# Inputs: 1) HiXray_YOLO2 dataset  2) this GitHub repo OR hybrid_rare_ft_epoch20.pt
# Accelerator: GPU (T4)
# What it does: validate released epoch20 checkpoint (per-class mAP50)
# =============================================================================
...
WORK = Path("/kaggle/working/hybrid_eval")
...
if DATASET is None:
    for p in INPUT.rglob("images/val"):
        ...
        if (root / "labels" / "val").exists():
            DATASET = root
            break
assert DATASET is not None, "Add HiXray_YOLO2 (images/val + labels/val) as Input"

# --- find checkpoint (repo clone or uploaded .pt) ---
CKPT = None
...
if CKPT is None:
    ...
    assert pts, "Add models/hybrid_rare_ft_epoch20.pt (clone repo or upload weights)"
    CKPT = pts[0]

# --- find / copy hybrid modules ---
MOD_SRC = None
for p in INPUT.rglob("hybrid_modules.py"):
    MOD_SRC = p.parent
    break
if MOD_SRC is not None:
    for f in ("hybrid_modules.py", "custom_rare_trainer.py", "register_hybrid.py"):
        src = MOD_SRC / f
        if src.exists():
            shutil.copy2(src, WORK / f)
else:
    # Fallback: write minimal modules (same as repo)
    print("WARNING: repo modules not found in Inputs — using embedded fallback")
...
if MARKER not in t:
    ...
    m = pat.search(t)
    assert m, "Could not patch ultralytics; pin ultralytics==8.4.103"
